...
public class StudentService {
    ...
    @Transactional
    public void updateStudent(Long studentId, Student studentin) {

        Student student = studentRepository.findById(studentId)
                .orElseThrow(()-> new IllegalStateException("Student with id = "+ studentId + " does not exist"));
        if(studentin!=null){
        if(studentin.getName() != null && studentin.getEmail()!= null) {
            if (student.getName() != null && studentin.getName().length() > 0 && !student.getName().equals(studentin.getName())) {
                student.setName(studentin.getName());

            }
            if (studentin.getEmail() != null && studentin.getEmail().length() > 0 && student.getEmail().equals(studentin.getEmail())) {
                Optional<Student> studentOptional = studentRepository.findStudentByEmail(studentin.getEmail());
